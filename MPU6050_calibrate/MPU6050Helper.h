...
const uint8_t dmp_memory[DMP_CODE_SIZE] PROGMEM = { //original MD6.12 from InvenSense
  /* bank # 0 */
  ...
  0xba, 0xa0, 0xdf, 0xdf, 0xdf, 0xd8, 0xd8, 0xf1, 0xb8, 0xaa, 0xb3, 0x8d, 0xb4, 0x98, 0x0d, 0x35, //enable gyro calibration
  ...
  0x87, 0x9c, 0xb9, 0xa3, 0xdd, 0xf1, 0xb3, 0x8b, 0x8b, 0x8b, 0x8b, 0x8b, 0xb0, 0x87, 0xa3, 0xa3, //enable quaternion somewhere around here
  0xa3, 0xa3, 0xb2, 0x8b, 0xb6, 0x9b, 0xf2, 0xa3, 0xa3, 0xa3, 0xa3, 0xa3, 0xa3, 0xa3, 0xa3, 0xa3, //all disabled by default
...
//based on variance of X gyro values
bool isResting(uint8_t threshold = 120) {
  uint32_t sum = 0;
  uint32_t sqsum = 0;
  const uint8_t dataPoints = 50;
  for (uint8_t i = 0; i < dataPoints; i++) {
    int16_t reading = readWord(mpuAddr, MPU6050_RA_GYRO_XOUT_H);
    sum += reading;
    sqsum += reading * reading;
    delay(10);
  }
  if (sqsum - (sum * sum / dataPoints) < threshold)
    return true;
  else
    return false;
}
...
int8_t mpu_write_mem(uint16_t mem_addr, uint16_t length, uint8_t *data) {
  ...
  if (!writeWord(mpuAddr, MPU6050_RA_BANK_SEL, mem_addr))
    return -1;
  if (!I2Cdev::writeBytes(mpuAddr, MPU6050_RA_MEM_R_W, length, data))
    return -1;
  return 0;
}
...
int8_t mpu_read_mem(uint16_t mem_addr, uint16_t length, uint8_t *data) {
  ...
  if (!writeWord(mpuAddr, MPU6050_RA_BANK_SEL, mem_addr))
    return -1;
  if (!I2Cdev::readBytes(mpuAddr, MPU6050_RA_MEM_R_W, length, data))
    return -1;
  return 0;
}
...
/**
    ...
    @return     0 if successful.
*/
int8_t mpu_load_firmware(uint16_t length, const uint8_t *firmware) {
  static bool loaded = false;
  uint16_t ii;
  uint16_t this_write;
  /* Must divide evenly into st.hw->bank_size to avoid bank crossings. */
  uint8_t firmware_chunk[MPU6050_DMP_MEMORY_CHUNK_SIZE];
#define VERIFY_WRITE
#ifdef VERIFY_WRITE
uint8_t cur[MPU6050_DMP_MEMORY_CHUNK_SIZE];
  ...
  for (ii = 0; ii < length; ii += this_write) {
    this_write = min(MPU6050_DMP_MEMORY_CHUNK_SIZE, length - ii);
    uint8_t *pFirmware = (uint8_t *)&firmware[ii];
    for (uint8_t x = 0; x < this_write; x++)
      firmware_chunk[x] = pgm_read_byte_near(firmware + x); //modify here to load from SD card
  ...
  if (!writeWord(mpuAddr, MPU6050_RA_DMP_CFG_1, DMP_START_ADDRESS))
    return -1;
